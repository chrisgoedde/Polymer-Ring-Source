...
def drawSpan(picturePathName, fname, title, span, zoneDict, colors, multi=False):
    plt.rc('text', usetex=True)

    # Create a figure using the plotting utilities already in the code base
    f, ax = plU.makeFigure(figsize=(6, 6), textSize=10, polar=True)

    # Set plot limits for the angular and radial components
    ax.set_xticks([np.pi / 3, 2 * np.pi / 3])
    ax.set_xticklabels(['Oblate', 'Prolate'])
    ax.set_thetamin(60)
    ax.set_thetamax(120)
    ...
    # If multi is true, then the function assumes that a
    # list of spans is passed as the "span" parameter.
    # It assumes a single span was passed if otherwise.
    if not multi:
        SiZones = set(span.S)

        for zone in SiZones:
            plU.drawWedge(ax, zoneDict[zone]['xmin'], zoneDict[zone]['xmax'], zoneDict[zone]['ymin'],
                          zoneDict[zone]['ymax'], lined=True,
                          theColor=colors)

        plt.title(title)

        plU.savePlot(picturePathName, fname, type='png', dpiArg=600, closeFigure=True)

    else:
        for i in range(0, len(span)):
            print(i)
            SiZones = set(span[i].S)
